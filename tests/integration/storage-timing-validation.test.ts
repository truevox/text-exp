...
describe('Storage Timing Validation - Critical Fix Tests', () => {
  ...
  describe('Critical Storage Timing Fix - syncNow()', () => {
    ...
    test('ERROR HANDLING: IndexedDB failure prevents content script notification', async () => {
      ...
      mockIndexedDB.saveSnippets.mockRejectedValue(new Error('IndexedDB write failed'));
  ...
  describe('Multi-Scope Sync Timing', () => {
    test('Complex merge operations maintain storage timing', async () => {
      const personalSnippets = [
        { id: '1', trigger: 'personal1', content: 'Personal content', createdAt: new Date(), lastModified: new Date(), isActive: true, scope: 'personal' }
      ];
      
      const departmentSnippets = [
        { id: '2', trigger: 'dept1', content: 'Department content', createdAt: new Date(), lastModified: new Date(), isActive: true, scope: 'department' }
      ];
      ...
      expect(callOrder).toEqual(['storage-update', 'indexedDB-update', 'notify-scripts']);
    });
  });
  ...
  describe('Performance and Timing Validation', () => {
    ...
    test('Concurrent sync attempts are properly handled', async () => {
      ...
      await expect(syncManager.syncNow()).rejects.toThrow('Sync already in progress');
