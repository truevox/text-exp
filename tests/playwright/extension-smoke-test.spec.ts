...
test.describe('PuffPuffPaste Extension - Smoke Tests', () => {
  ...
  test('should have valid manifest and extension structure', async () => {
    ...
    try {
      ...
      if (backgroundPages.length > 0) {
        const backgroundPage = backgroundPages[0];
        const extensionId = backgroundPage.url().match(/chrome-extension:\/\/([^/]+)/)?.[1];
        ...
        if (extensionId) {
          ...
          try {
            ...
            console.log('Manifest check note:', e.message);
          } finally {
  ...
  test('should allow basic page interaction', async () => {
    ...
    try {
      ...
      await page.goto('data:text/html,<html><body><input type="text" id="test" placeholder="Type here..."><div>Extension smoke test page</div></body></html>');
      
      // Verify page loaded
      const pageTitle = await page.evaluate(() => document.title || 'Test Page');
  ...
  test('should handle extension popup loading', async () => {
    ...
    try {
      ...
      if (backgroundPages.length > 0) {
        const extensionId = backgroundPages[0].url().match(/chrome-extension:\/\/([^/]+)/)?.[1];
        ...
        if (extensionId) {
          ...
          try {
            ...
            console.log('Popup load note:', e.message);
