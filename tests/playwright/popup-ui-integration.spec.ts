...
test.describe("PuffPuffPaste Popup UI Integration", () => {
  test.beforeAll(async () => {
    ...
    if (backgroundPages.length > 0) {
      extensionId =
        backgroundPages[0].url().match(/chrome-extension:\/\/([^/]+)/)?.[1] ||
        "";
    } else {
      const backgroundPage = await context.waitForEvent("backgroundpage");
      extensionId =
        backgroundPage.url().match(/chrome-extension:\/\/([^/]+)/)?.[1] || "";
    }
  ...
  test("should display main UI elements", async () => {
    ...
    const elements = await popupPage.evaluate(() => {
      return {
        ...
        hasHeadings:
          document.querySelectorAll('h1, h2, h3, .title, [class*="title"]')
            .length > 0,
    ...
    expect(
      elements.hasButtons || elements.hasInputs || elements.hasHeadings,
    ).toBe(true);
  });

  test("should handle snippet search functionality", async () => {
    // Look for search input
    const searchInputs = await popupPage
      .locator(
        'input[type="text"], input[placeholder*="search"], input[id*="search"], input[class*="search"]',
      )
      .count();
    ...
    if (searchInputs > 0) {
      ...
      console.log(
        "No search input found in popup - this is optional functionality",
      );
    }
  });
  ...
  test("should handle snippet creation form", async () => {
    ...
    const triggerInputs = await popupPage
      .locator(
        'input[placeholder*="trigger"], input[id*="trigger"], input[name*="trigger"]',
      )
      .count();
    const contentInputs = await popupPage
      .locator('textarea, input[placeholder*="content"], input[id*="content"]')
      .count();

    if (forms > 0 || (triggerInputs > 0 && contentInputs > 0)) {
      // Test snippet creation form if present
      if (triggerInputs > 0) {
        const triggerInput = popupPage
          .locator(
            'input[placeholder*="trigger"], input[id*="trigger"], input[name*="trigger"]',
          )
          .first();
      ...
      if (contentInputs > 0) {
        const contentInput = popupPage
          .locator(
            'textarea, input[placeholder*="content"], input[id*="content"]',
          )
          .first();
      ...
      console.log(
        "No snippet creation form found - testing basic popup functionality",
      );
    }
  });
  ...
  test("should handle settings and preferences", async () => {
    ...
    const settingsElements = await popupPage
      .locator(
        '[class*="setting"], [id*="setting"], input[type="checkbox"], select',
      )
      .count();

    if (settingsElements > 0) {
      // Test checkbox interactions
      const checkboxes = await popupPage
        .locator('input[type="checkbox"]')
        .all();

      for (const checkbox of checkboxes.slice(0, 2)) {
        // Test first 2 checkboxes
      ...
      for (const select of selects.slice(0, 1)) {
        // Test first select
  ...
  test("should handle snippet list display", async () => {
    ...
    const listElements = await popupPage
      .locator('ul, ol, .list, [class*="snippet"], [id*="snippet"]')
      .count();

    if (listElements > 0) {
      // Test snippet list interactions
      const snippetItems = await popupPage
        .locator('li, .snippet-item, [class*="snippet-item"]')
        .count();

      if (snippetItems > 0) {
        // Test first snippet item
        const firstItem = popupPage
          .locator('li, .snippet-item, [class*="snippet-item"]')
          .first();
      ...
      console.log(
        "No snippet list found - testing completed popup functionality",
      );
    }
  });
  ...
  test("should maintain responsive design", async () => {
    ...
    for (const viewport of viewports) {
      ...
      const bodyHeight = await popupPage.evaluate(
