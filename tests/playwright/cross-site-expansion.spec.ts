...
const TEST_SITES: TestSite[] = [
  ...
  },
  {
    ...
    altInputSelectors: [".ace_editor", ".monaco-editor textarea"],
    expectedBehavior: "paste-fallback",
    notes: "ACE editor - requires special paste strategy",
  },
  {
    name: "JSFiddle JavaScript",
    url: "https://jsfiddle.net/",
    inputSelector: ".ace_text-input",
    altInputSelectors: [".ace_editor", ".monaco-editor textarea", "#id_code_js"],
    expectedBehavior: "paste-fallback",
  ...
  },
  {
    ...
    altInputSelectors: ["#issue_body", "textarea[name='issue[body]']"],
    expectedBehavior: "expansion",
  ...
  },
  {
    ...
    altInputSelectors: ["[data-testid='comment-body-textarea']", "textarea"],
    expectedBehavior: "expansion",
  ...
  },
  {
    ...
    inputSelector: "[contenteditable='true']",
    altInputSelectors: ["textarea", ".public-DraftEditor-content"],
    expectedBehavior: "expansion",
...
const TEST_SNIPPETS: TestSnippet[] = [
  ...
  },
  {
    ...
    variables: [
      ...
      { name: "company", description: "Company name", defaultValue: "Acme Corp" },
    ],
  },
];
...
test.beforeEach(async ({ context }) => {
  ...
  try {
    const extensionPages = context.pages().filter(p => p.url().includes('chrome-extension://'));
  ...
  try {
    ...
    for (const snippet of TEST_SNIPPETS.slice(0, 4)) { // Limit to first 4 snippets for faster setup
      try {
...
for (const site of TEST_SITES) {
  ...
  test(`Cross-site expansion: ${site.name}`, async ({ context }) => {
    ...
    try {
      ...
      await takeDebugScreenshot(page, `${site.name.replace(/\s+/g, '-')}-initial`);
      ...
      if (site.inputSelector) {
        try {
          ...
          console.warn(`Primary selector ${site.inputSelector} not found, trying alternatives`);
      ...
      for (const snippet of TEST_SNIPPETS.slice(0, 3)) { // Test first 3 snippets to keep test time reasonable
        ...
        try {
          ...
          if (site.expectedBehavior === "expansion") {
            ...
            console.log(`✅ Successful expansion on ${site.name}: ${snippet.trigger} → content`);
            ...
            if (hasContent) {
              console.log(`✅ Paste fallback worked on ${site.name}: content inserted`);
            } else if (!hasContent && hasTrigger) {
              console.log(`⚠️ Expansion failed on ${site.name}, trigger still present: ${elementContent.substring(0, 100)}`);
              // This is expected for some complex sites - log but don't fail
            } else {
              console.log(`❓ Unclear expansion state on ${site.name}: ${elementContent.substring(0, 100)}`);
            }
          ...
          console.error(`❌ Snippet test failed for ${snippet.trigger} on ${site.name}:`, snippetError);
          ...
          console.log(`⚠️ Continuing despite error - this may be expected for ${site.name}`);
        }
      ...
      // Take error screenshot
      await takeDebugScreenshot(page, `${site.name.replace(/\s+/g, '-')}-site-error`);
      ...
      if (site.name.includes("CodePen") || site.name.includes("JSFiddle")) {
        console.log(`⚠️ Site ${site.name} failed to load properly - this may be expected`);
      } else {
        throw siteError;
      }
    } finally {
  ...
  for (const input of testInputs) {
    ...
    try {
      ...
      if (input.name === "Broken Editor") {
        ...
        if (!cleanContent.includes("הרבה") && !cleanContent.includes("מקולקל")) {
...
test("Cross-site expansion: Network resilience", async ({ context }) => {
  ...
  for (const url of unreachableSites) {
    try {
      ...
      console.log(`✅ Correctly handled network error for ${url}: ${error.message}`);
    }
  }
...
test("Cross-site expansion: Paste strategy stress test", async ({ context }) => {
  const page = await context.newPage();
  ...
  const testSelectors = ["#rapid-input", "#long-content", "#multi-ce-1", "#multi-ce-2", "#dynamic-content"];
  ...
  for (let round = 0; round < 3; round++) {
    ...
    for (const selector of testSelectors) {
      try {
        ...
        const testTrigger = round === 0 ? "hello" : round === 1 ? "signature" : "code-snippet";
        ...
        console.log(`📝 Round ${round + 1}, ${selector}: ${content.substring(0, 50)}...`);
        
      } catch (error) {
        console.warn(`⚠️ Stress test error on ${selector}, round ${round + 1}:`, error.message);
      }
    }
