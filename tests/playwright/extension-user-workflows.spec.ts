...
test.describe('PuffPuffPaste Extension - Real User Workflows', () => {
  ...
  test.beforeEach(async () => {
    ...
    await testPage.goto('data:text/html,<html><body><input type="text" id="test-input" style="width:100%;height:50px;font-size:16px;" placeholder="Type here to test text expansion..."><textarea id="test-textarea" style="width:100%;height:100px;font-size:16px;margin-top:10px;" placeholder="Test area for text expansion..."></textarea></body></html>');
  });
  ...
  test('should open options page and display settings', async () => {
    ...
    await expect(optionsPage.locator('h1, .title, [class*="title"]')).toBeVisible();
    
    // Check for settings sections
    const settingsElements = await optionsPage.locator('input, select, button').count();
  ...
  test('should work in contenteditable elements', async () => {
    ...
    await testPage.evaluate(() => {
      ...
      div.style.cssText = 'width:100%;height:100px;border:1px solid #ccc;padding:10px;font-size:16px;margin-top:10px;';
  ...
  test('should handle extension popup interactions', async () => {
    ...
    try {
      ...
      await popupPage.screenshot({ path: 'tests/playwright-report/popup-screenshot.png' });
      
    } catch (error) {
      console.log('Popup test note:', error.message);
...
test.describe('Extension Context Validation', () => {
  ...
  test('should have extension APIs available in content script context', async () => {
    ...
    await page.goto('data:text/html,<html><body><h1>Extension API Test</h1></body></html>');
