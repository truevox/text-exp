...
const MANUAL_TEST_SITES: ManualTestSite[] = [
  {
    ...
    url: "https://text.new/",
    instructions:
      "Click in the editor and type 'hello' - should expand to 'Hello, World!'",
  ...
  },
  {
    ...
    instructions:
      "Click in the JavaScript panel and type 'hello' - may require paste fallback",
  ...
  },
  {
    ...
    instructions:
      "Click in the title field and type 'hello' - should expand normally",
  ...
  },
  {
    ...
    instructions:
      "Type 'hello' in either input - should expand to 'Hello, World!'",
...
test.describe("Manual Cross-Site Expansion Tests", () => {
  test("Extension availability check", async ({ page }) => {
    ...
    await page.goto(
      "data:text/html,<input type='text' id='test-input' placeholder='Type hello to test extension'>",
    );
    ...
    if (value.includes("Hello, World!")) {
      ...
      console.log(
        "❌ Extension may not be loaded. Trigger text remains unchanged.",
      );
      console.log(
        "💡 Make sure PuffPuffPaste extension is installed and enabled.",
      );
    } else {
  ...
  test("Automated input detection test", async ({ page }) => {
    ...
    for (const input of testInputs) {
      ...
      try {
        ...
        // Check the result
        const value =
          input.selector === "#contenteditable"
  ...
  test("Cross-site compatibility report", async ({ page }) => {
    ...
    const compatibilityTests = [
      ...
      {
        site: "Gmail-style",
        url: "data:text/html,<div contenteditable='true' style='border:1px solid #ccc; padding:10px;'>Type here</div>",
      },
      {
        site: "Code editor-style",
        url: "data:text/html,<textarea style='font-family:monospace; background:#1e1e1e; color:#d4d4d4; padding:10px;'></textarea>",
      },
    ];
    ...
    for (const testCase of compatibilityTests) {
      ...
      try {
        ...
        console.log(
          `📊 ${testCase.site}: ${testResult.expansionWorked ? "✅ PASS" : "❌ FAIL"}`,
        );
      } catch (error) {
    ...
    report.results.forEach((result, index) => {
      ...
      if (result.error) {
        ...
        console.log(
          `   Status: ${result.expansionWorked ? "✅ PASS" : "❌ FAIL"}`,
        );
    ...
    console.log(
      `📊 Summary: ${passCount}/${totalCount} tests passed (${Math.round((passCount / totalCount) * 100)}%)`,
    );
