...
describe('TextReplacer', () => {
  ...
  describe('Text Insertion', () => {
    ...
    test('should insert text at cursor in contenteditable', () => {
      ...
      global.document.createTextNode = jest.fn(() => mockTextNode);
  ...
  describe('Cursor Position Management', () => {
    ...
    test('should set cursor position in contenteditable', () => {
      ...
      global.document.createTreeWalker = jest.fn(() => mockWalker);
  ...
  describe('Error Handling', () => {
    test('should handle unsupported element types', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Unsupported element type for text replacement');
    ...
    test('should handle errors during replacement', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Error replacing text:', expect.any(Error));
      consoleSpy.mockRestore();
    });
    ...
    test('should handle errors during event triggering', () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Error triggering input event:', expect.any(Error));
      consoleSpy.mockRestore();
    });
  });
  ...
  describe('Range Creation for ContentEditable', () => {
    test('should create range from text offsets', () => {
      ...
      const replacerAny = replacer as any;
      const range = replacerAny.createRangeFromOffsets(mockContentEditable, 3, 7);
    ...
    test('should handle range creation errors', () => {
      ...
      const replacerAny = replacer as any;
      const range = replacerAny.createRangeFromOffsets(mockContentEditable, 0, 5);

      expect(range).toBeNull();
      expect(consoleSpy).toHaveBeenCalledWith('Error creating range:', expect.any(Error));
      consoleSpy.mockRestore();
    });
