...
describe("Word Boundary Detection Bug - 'wub' inside 'Wubbalubba'", () => {
  ...
  describe("Cases that should NOT match 'wub' trigger", () => {
    const shouldNotMatch = [
      { input: "Wubbalubba", description: "wub inside Wubbalubba - CRITICAL BUG" },
      { input: "Wubbalubba Dub Dub!!!", description: "Full text from user logs" },
      { input: "wubba", description: "wub at start but followed by alphanumeric" },
      { input: "subwub", description: "wub at end but preceded by alphanumeric" },
    ...
    shouldNotMatch.forEach(({ input, description }) => {
      test(`should NOT match "wub" in "${input}" (${description})`, () => {
        ...
        // Log for debugging
        console.log(`❌ "${input}" -> Match: ${result.isMatch}, State: ${result.state}, Trigger: ${result.trigger || "none"}`);
      });
    });
  });
  ...
  describe("Cases that SHOULD match 'wub' trigger", () => {
    const shouldMatch = [
      ...
      { input: "test wub test", description: "Surrounded by proper boundaries" },
    ...
    shouldMatch.forEach(({ input, description }) => {
      test(`should match "wub" in "${input}" (${description})`, () => {
        ...
        // Log for debugging
        console.log(`✅ "${input}" -> Match: ${result.isMatch}, State: ${result.state}, Trigger: ${result.trigger}`);
      });
    });
  });
  ...
  describe("Specific user-reported case", () => {
    test('should NOT match "wub" inside "Wubbalubba Dub Dub!!!" - CRITICAL BUG REPRODUCTION', () => {
      ...
      console.log(`   Result: Match=${result.isMatch}, State=${result.state}, Trigger=${result.trigger || "none"}`);
      console.log(`   Expected: Match=false (wub should NOT be found inside Wubbalubba)`);
    });
  });
});