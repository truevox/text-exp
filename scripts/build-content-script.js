...
async function buildContentScriptAsIIFE() {
  ...
  try {
    ...
    const iifeFile = path.join(projectRoot, 'build-temp/content-script.iife.js');
    const iifeContent = fs.readFileSync(iifeFile, 'utf8');
    
    // Copy it to the main build directory
    const outputFile = path.join(projectRoot, 'build/content/content-script.js');
    fs.mkdirSync(path.dirname(outputFile), { recursive: true });
    fs.writeFileSync(outputFile, iifeContent);
    
    // Clean up temp directory
    fs.rmSync(path.join(projectRoot, 'build-temp'), { recursive: true, force: true });
