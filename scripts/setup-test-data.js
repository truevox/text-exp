    ...
    SAMPLE_SNIPPETS.forEach((snippet) => {
      const vars =
        snippet.variables.length > 0
          ? ` (${snippet.variables.length} variables)`
          : "";
      console.log(
        `  ${snippet.trigger} - ${snippet.content.split("\n")[0].substring(0, 50)}...${vars}`,
      );
    });
...
if (typeof chrome !== "undefined" && chrome.storage) {
  console.log(
    "🔧 Test data setup script loaded. Run setupTestData() to begin.",
  );
} else {
  console.log(
    "⚠️  Chrome extension APIs not available. Load this script in extension context.",
  );
}
