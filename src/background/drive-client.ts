...
export class DriveClient {
  ...
  static async uploadSnippets(snippets: TextSnippet[]): Promise<SyncResult> {
    try {
      ...
      if (!result.ok) {
        ...
        throw new Error(
          `Drive API error: ${errorData.error?.message || result.statusText}`,
        );
      }
      ...
      console.log(
        `✅ Snippets uploaded successfully (${snippets.length} snippets)`,
      );
      return {
        success: true,
        snippets,
  ...
  static async downloadSnippets(): Promise<SyncResult> {
    try {
      ...
      const response = await fetch(
        `https://www.googleapis.com/drive/v3/files/${snippetFile.id}?alt=media`,
        {
          headers: {
      ...
      console.log(
        `✅ Downloaded ${data.snippets.length} snippets from Google Drive`,
      );
      return {
        success: true,
   ...
   */
  private static async findSnippetFile(
    accessToken: string,
  ...
  private static async createFile(
    ...
    form.append(
      "file",
      new Blob([JSON.stringify(snippetData, null, 2)], {
        type: this.MIME_TYPE,
      }),
    );
   ...
   */
  static async getFileMetadata(): Promise<{
    lastModified?: string;
    size?: number;
  } | null> {
    try {
  ...
  static async testConnection(): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (!response.ok) {
        return {
          success: false,
          error: `API test failed: ${response.statusText}`,
        };
      }

      const data = await response.json();
      console.log(
        "✅ Drive API connection successful:",
   ...
   */
  static async deleteSnippetFile(): Promise<{
    success: boolean;
    error?: string;
  }> {
    try {
      ...
      const response = await fetch(
        `https://www.googleapis.com/drive/v3/files/${snippetFile.id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${accessToken}`,
