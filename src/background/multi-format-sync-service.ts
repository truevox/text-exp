...
interface FileDiscoveryAdapter extends CloudAdapter {
   ...
   */
  listFiles?(folderId?: string): Promise<Array<{
    id: string;
    name: string;
    mimeType?: string;
    modifiedTime?: string;
...
export class MultiFormatSyncService {
   ...
   * Check if adapter supports file discovery
   */
  private supportsFileDiscovery(adapter: CloudAdapter): adapter is FileDiscoveryAdapter {
    return typeof (adapter as FileDiscoveryAdapter).listFiles === 'function' &&
           typeof (adapter as FileDiscoveryAdapter).downloadFileContent === 'function';
  }
  ...
  private async downloadFromMultipleFiles(
    ...
    try {
      console.log(`🔍 Discovering snippet files in folder: ${folderId || 'root'}`);
      ...
      for (const file of snippetFiles) {
        ...
        try {
          ...
          console.log(`🔄 Parsing file: ${file.name} (${content.length} chars)`);
          ...
          console.log(`✅ Parsed ${snippets.length} snippets from ${file.name}`);
        } catch (error) {
      ...
      console.log(`🎉 Successfully loaded ${allSnippets.length} snippets from ${snippetFiles.length} files`);
      return allSnippets;
  ...
  private isSnippetFile(fileName: string): boolean {
    ...
    const hasSnippetName = lowerName.includes('snippet') || 
   ...
   */
  private convertDocsToSnippets(docs: SnippetDoc[], sourceFile: string): TextSnippet[] {
    return docs.map(doc => ({
      id: doc.meta.id,
  ...
  async uploadSnippetsWithFormats(
    ...
    folderId?: string,
    preferredFormat: 'json' | 'txt' | 'md' | 'html' | 'tex' = 'json'
  ): Promise<void> {
