...
export class LaTeXParser implements FormatParser {
  ...
  private parseWithFrontmatter(content: string, fileName?: string): SnippetDoc {
    ...
    const fullMeta: SnippetMeta = {
      id: meta.id || `tex-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  private parseWithoutFrontmatter(content: string, fileName?: string): SnippetDoc {
    ...
    const meta: SnippetMeta = {
      ...
      snipDependencies: [],
      contentType: 'latex',
      description: title || `LaTeX snippet${fileName ? ` from ${fileName}` : ''}`,
      scope: 'personal',
  ...
  private extractVariables(latex: string): VariableDef[] {
    ...
    while ((match = variableRegex.exec(latex)) !== null) {
      ...
      if (!this.isLaTeXCommand(variable) && !variable.includes('\\') && !variable.includes(' ')) {
  ...
  private isLaTeXCommand(text: string): boolean {
    ...
    const latexConstructs = [
      'document', 'theorem', 'proof', 'equation', 'align', 'figure', 'table',
      'center', 'itemize', 'enumerate', 'description', 'abstract', 'section',
      'subsection', 'chapter', 'part', 'paragraph', 'subparagraph'
    ];
    
    return latexConstructs.includes(text.toLowerCase()) || 
           text.startsWith('\\') || 
           /^[a-z]+\*?$/i.test(text); // Single word commands
  }
  ...
  private normalizeVariables(variables: any, content?: string): VariableDef[] {
    ...
    if (content) {
      ...
      while ((match = variableRegex.exec(content)) !== null) {
        const name = match[1];
        if (!this.isLaTeXCommand(name) && !name.includes('\\') && !variableMap.has(name)) {
  ...
  createNew(trigger: string, content: string, options: Partial<SnippetMeta> = {}): SnippetDoc {
    const now = new Date().toISOString();
    const id = options.id || `tex-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
