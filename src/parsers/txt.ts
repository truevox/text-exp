...
import type { FormatParser, SnippetDoc, SnippetMeta, ParseOptions, SerializeOptions } from '../types/snippet-formats.js';
...
export class TxtParser implements FormatParser {
  ...
  private parseWithFrontmatter(content: string, fileName?: string): SnippetDoc {
    ...
    const fullMeta: SnippetMeta = {
      id: meta.id || `txt-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  private serializeOne(doc: SnippetDoc, options: SerializeOptions = {}): string {
    ...
    if (meta.language && meta.language !== 'text') yamlMeta.language = meta.language;
    if (meta.category) yamlMeta.category = meta.category;
    if (meta.scope && meta.scope !== 'personal') yamlMeta.scope = meta.scope;
    if (meta.isActive === false) yamlMeta.isActive = false;
    if (meta.priority !== 0) yamlMeta.priority = meta.priority;
  ...
  private normalizeVariables(variables: any): { name: string; prompt: string }[] {
  ...
  createNew(trigger: string, content: string, options: Partial<SnippetMeta> = {}): SnippetDoc {
    const now = new Date().toISOString();
    const id = options.id || `txt-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
