...
import type { FormatParser, SnippetDoc, SnippetMeta, VariableDef, LegacySnippet, ParseOptions, SerializeOptions } from '../types/snippet-formats.js';
...
export class JSONParser implements FormatParser {
  ...
  private parseSnippetItem(item: any, index: number): SnippetDoc {
    ...
    if (!item.meta || !item.body) {
      throw new Error(`Snippet at index ${index} is missing required fields (meta, body)`);
    }

    const meta: SnippetMeta = {
      id: item.meta.id || `snippet-${Date.now()}-${index}`,
      trigger: item.meta.trigger,
      snipDependencies: Array.isArray(item.meta.snipDependencies) ? item.meta.snipDependencies : [],
  ...
  private convertLegacySnippet(legacy: LegacySnippet): SnippetDoc {
    const meta: SnippetMeta = {
      ...
      images: [],
      tags: [],
      createdAt: legacy.createdAt?.toISOString() || new Date().toISOString(),
      createdBy: 'legacy-import',
      updatedAt: (legacy.lastModified || legacy.createdAt || new Date()).toISOString(),
  ...
  serialize(docs: SnippetDoc | SnippetDoc[], options: SerializeOptions = {}): string {
      ...
      items.forEach((item, index) => {
        ...
        if (!item.meta) {
          ...
          if (typeof item.meta.trigger !== 'string') {
            errors.push(`Snippet at index ${index} has invalid meta.trigger (must be string)`);
          }
        }
  ...
  createNew(trigger: string, content: string, options: Partial<SnippetMeta> = {}): SnippetDoc {
    ...
    const id = options.id || `snippet-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
