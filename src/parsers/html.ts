...
export class HTMLParser implements FormatParser {
  ...
  private extractHTMLFrontmatter(
    content: string,
  ...
  private parseWithFrontmatter(
    content: string,
    extracted: { frontmatter: string; body: string },
    fileName?: string,
    options: ParseOptions = {},
  ): SnippetDoc {
    let meta: any;
    ...
    const fullMeta: SnippetMeta = {
      id:
        meta.id ||
        `html-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  private parseWithoutFrontmatter(
    ...
    const meta: SnippetMeta = {
      ...
      contentType: "html",
      description:
        title || `HTML snippet${fileName ? ` from ${fileName}` : ""}`,
      scope: "personal",
  ...
  private validateHTMLSyntax(html: string, errors: string[]): void {
    ...
    while ((match = tagRegex.exec(html)) !== null) {
      ...
      const isSelfClosing =
        match[0].endsWith("/>") || selfClosingTags.test(tagName);
