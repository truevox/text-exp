 ...
 */
export function detectFormat(fileName: string, fileContent: string): SnippetFormat {
  ...
  if (/\\begin\{document\}|\\documentclass|\\usepackage|\\begin\{.*\}|\\end\{.*\}|\\[a-zA-Z]+/i.test(content)) {
    return 'tex';
  }
  
  // HTML format detection
  if (lowerFileName.endsWith('.html') || lowerFileName.endsWith('.htm') || lowerFileName.endsWith('.ppp.html')) {
    return 'html';
  }
  
  // Check for HTML tags (case insensitive)
  if (/<html|<head|<body|<p|<div|<span|<h[1-6]|<ul|<ol|<li|<table|<tr|<td|<img|<a\s/i.test(content)) {
    return 'html';
  }
  
  // Markdown format detection
  if (lowerFileName.endsWith('.md') || lowerFileName.endsWith('.markdown') || lowerFileName.endsWith('.ppp.md')) {
    return 'md';
  }
 ...
 */
export function extractYAMLFrontmatter(content: string): { frontmatter: string; body: string } | null {
