 ...
 */
const EXCLUDED_FIELDS = ['id', 'createdAt', 'createdBy', 'updatedAt', 'updatedBy'] as const;
 ...
 */
export function generateSnippetHash(snippet: SnippetMeta | EnhancedSnippet): string {
  // Create a copy excluding timestamp and ID fields
  const hashableData = Object.fromEntries(
    Object.entries(snippet).filter(([key]) => !EXCLUDED_FIELDS.includes(key as any))
  );
 ...
 */
export function generatePartialSnippetHash(partialSnippet: Partial<SnippetMeta | EnhancedSnippet>): string {
  // Filter out excluded fields from partial data
  const hashableData = Object.fromEntries(
    Object.entries(partialSnippet).filter(([key]) => !EXCLUDED_FIELDS.includes(key as any))
  );
 ...
 */
export function getHashableFields(snippet: SnippetMeta | EnhancedSnippet): Partial<SnippetMeta | EnhancedSnippet> {
