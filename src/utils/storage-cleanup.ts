...
export class StorageCleanup {
   ...
   */
  static async clearInvalidSources(): Promise<{
    cleaned: number;
    errors: string[];
  }> {
    const errors: string[] = [];
    ...
    try {
      ...
      if (settings.configuredSources) {
        const validSources = settings.configuredSources.filter((source) => {
          if (source.provider === "local-filesystem") {
            cleaned++;
            console.log(
              `Removing invalid local filesystem source: ${source.displayName}`,
            );
            return false;
          }
          return true;
      ...
      if (scopedSources && scopedSources.length > 0) {
        const validScopedSources = scopedSources.filter((source) => {
          if (source.provider === "local-filesystem") {
            cleaned++;
            console.log(
              `Removing invalid scoped source: ${source.displayName}`,
            );
            return false;
          }
          return true;
      ...
      const errorMsg = `Failed to clean storage: ${error instanceof Error ? error.message : String(error)}`;
   ...
   */
  static async validateAndCleanSources(): Promise<{
    valid: number;
    invalid: number;
    errors: string[];
  }> {
    ...
    try {
      ...
      const validSources = settings.configuredSources.filter((source) => {
        ...
        if (!supportedProviders.includes(source.provider)) {
          invalid++;
          console.log(
            `Removing source with unsupported provider: ${source.provider}`,
          );
          return false;
        }
      ...
      if (invalid > 0) {
        ...
        console.log(
          `Validation complete: ${valid} valid, ${invalid} invalid sources cleaned`,
        );
      }

      return { valid, invalid, errors };
    } catch (error) {
      const errorMsg = `Failed to validate sources: ${error instanceof Error ? error.message : String(error)}`;
      console.error(errorMsg, error);
      errors.push(errorMsg);
      return { valid, invalid, errors };
    }
  ...
  static async getCleanupStatus(): Promise<{
    ...
    try {
      ...
      if (settings.configuredSources) {
        const localFsSources = settings.configuredSources.filter(
          (s) => s.provider === "local-filesystem",
        );
        invalidSources += localFsSources.length;

        if (localFsSources.length > 0) {
          recommendations.push(
            `Remove ${localFsSources.length} local filesystem sources`,
          );
        }
      }
